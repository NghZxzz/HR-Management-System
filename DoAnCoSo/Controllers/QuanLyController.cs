...
namespace DoAnCoSo.Controllers
{
    ...
    public class QuanLyController : Controller
    {
        ...
        public async Task<IActionResult> QuanLyNV(int page =1)
        {
            int pageSize = 3;
            page = page < 1 ? 1 : page ;
            var nhanViens =  _context.NhanViens.Include(x => x.ChucVu).OrderBy(i => i.MaNhanVien).ToPagedList(page, pageSize);
        ...
        [Authorize(Roles = SD.Role_Admin)]
        public async Task<IActionResult> QuanLyTK(int page=1)
        {
            int pageSize = 3;
            page = page < 1 ? 1 : page;
            var user = _context.Users.Include(x => x.NhanVien).OrderBy(i => i.MaNhanVien).ToPagedList(page, pageSize);
            return View(user);
        }
        ...
        [Authorize(Roles = SD.Role_Admin)]
        public async Task<IActionResult> QuanLyNP(int page=1)
        {
            int pageSize = 10;
            page = page < 1 ? 1 : page;
            var nghiPheps = _context.NghiPheps.OrderByDescending(i=>i.Id).Include(x => x.NhanViens).ToPagedList(page, pageSize);
            return View(nghiPheps);
        }
        public async Task<IActionResult> QuanLyNPUser(int id)
        {
            var nghiPheps = await _context.NghiPheps.OrderByDescending(i => i.Id).Include(x => x.NhanViens).Where(t => t.MaNhanVien == id).ToListAsync();
            return View(nghiPheps);
        }
        ...
        public async Task<IActionResult> CreateLuongNV(BangLuong bangLuong)
        {
            ...
            if (ModelState.IsValid)
            {
                var s = await _context.bangLuongs.Where(x => x.MaNhanVien == bangLuong.MaNhanVien).ToListAsync();
                if (s!= null)
                {
                    TempData["danger"] = "Nhân viên này đã tồn tại bảng lương!!!";
                    return RedirectToAction(nameof(CreateLuongNV));
                }
